...
def backExamRegister():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""SELECT * FROM "back_ExamStyleSetting" """)
            result = cur.fetchall()
    except Exception as e:
        result=None
        print('Error')
    return importPackage.render_template('後台網頁/back_02_Registration_Management/back_01_ExamStudent_Register/back_ExamStyleSetting.html', result=result)
...
def backExamRegisterDelete():
    ...
    if importPackage.request.method == "POST":
        ...
        try:
            ...
            result=None
            print('Error')
        return importPackage.render_template('後台網頁/back_02_Registration_Management/back_01_ExamStudent_Register/back_ExamStyleSetting.html',Resetresult=result)