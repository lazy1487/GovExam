...
def backAgencyInfoModify():
    ...
    oldAgencyDepartmentID = str(importPackage.request.args.get('AgencyDepartmentID'))
    ...
    if importPackage.request.method == "POST":
        ...
        try:
            ...
            cursor.close()
            pool.putconn(conn)
        return importPackage.render_template('後台網頁/back_02_Registration_Management/back_02_Agency_Register/back_AgencyInfoModify.html', result=result, Handler=user, DateTime=formatted_time)
    else:
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    """SELECT * FROM "back_AgencyInfo" WHERE "AgencyDepartmentID" = %s""", (oldAgencyDepartmentID,))
                result = cursor.fetchall()
        except Exception as e:
            result = None
            print('ErrorB')
            ...
            cursor.close()
            pool.putconn(conn)
        return importPackage.render_template('後台網頁/back_02_Registration_Management/back_02_Agency_Register/back_AgencyInfoModify.html', result=result, Handler=user, DateTime=formatted_time, s=oldAgencyDepartmentID)
