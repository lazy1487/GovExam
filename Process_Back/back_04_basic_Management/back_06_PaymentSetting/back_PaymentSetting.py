    ...
    if importPackage.request.method == "POST":
        ...
        try:
           with conn.cursor() as cursor:
            ...
            conn.close()
            pool.putconn(conn)
        return importPackage.render_template('後台網頁/back_04_basic_Management/back_06_PaymentSetting/back_PaymentSetting.html', result=result,Handler=user, DateTime=formatted_time)
    else:
        try:
            with conn.cursor() as cursor:
                cursor = conn.cursor()
