...
back_NormalQuestionDesModify = importPackage.Blueprint(
    'back_NormalQuestionDesModify', __name__)
...
def backNormalQuestionDesModify():
    ...
    if importPackage.request.method == "POST":
        ...
        try:
            ...
            cursor.execute(
                """SELECT * FROM "back_NormalDesQuestion" WHERE "normalQuestionDesTitle" = %s """, (normalQuestionDesTitle,))
            result = cursor.fetchall()
        finally:
            cursor.close()
            pool.putconn(conn)

        return importPackage.render_template('後台網頁/back_04_basic_Management/back_04_NormalQuestionSetting/back_NormalQuestionDesModify.html', result=result, user_name=user_name, DateTime=formatted_time)
    else:
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    """SELECT * FROM "back_NormalDesQuestion" WHERE "normalQuestionDesTitle" = %s """, (Title,))
                result = cursor.fetchall()
        ...
        return importPackage.render_template('後台網頁/back_04_basic_Management/back_04_NormalQuestionSetting/back_NormalQuestionDesModify.html', result=result, Title=Title, Style=Style, user_name=user_name,  DateTime=formatted_time)
