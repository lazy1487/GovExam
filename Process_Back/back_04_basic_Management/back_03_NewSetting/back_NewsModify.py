...
def backNewsModify():
    ...
    if importPackage.request.method=="POST":
        ...
        try:
            ...
            cursor.execute("""SELECT * FROM "back_NewsSetting" WHERE "inputSubTitle" = %s """,(inputSubTitle,))
            result = cursor.fetchall()
        finally:
            cursor.close()
            pool.putconn(conn)
        
        return importPackage.render_template('後台網頁/back_04_basic_Management/back_03_NewSetting/back_NewsModify.html',result=result,Handler=user,DateTime=formatted_time )
    else:
        try:
            with conn.cursor() as cursor:
                cursor.execute("""SELECT * FROM "back_NewsSetting" WHERE "inputSubTitle" = %s""",(Title,))
                result = cursor.fetchall()
        except Exception as e:
            result = None
            print('Error')
            ...
            cursor.close()
            pool.putconn(conn)
        return importPackage.render_template('後台網頁/back_04_basic_Management/back_03_NewSetting/back_NewsModify.html',result=result,Handler=user,DateTime=formatted_time,Title=Title)
...
def backNewsModifyDelete():
    ...
    if importPackage.request.method == 'POST':
        ...
        return importPackage.render_template('後台網頁/back_04_basic_Management/back_03_NewSetting/back_NewsSetting.html', result=result,Title=Title)
